...
@jax.jit
def compute_audio_strengths_for_a_chunk(camera_pixel_microphone_time_offsets, data_fft, data_frequencies):
    ...
    audio_strengths = jax.numpy.mean(jax.numpy.square(jax.numpy.abs(data_fft_shifted_summed)), axis=1) / number_of_microphones
...
def compute_audio_strengths(microphone_samples, n, microphone_sample_rate, camera_pixel_microphone_time_offsets, pixel_chunk_size):
    data_fft = jax.numpy.fft.fft(microphone_samples, axis=0)
    ...
    for delimiter, size in zip(delimiters, sizes):
        audio_strengths = audio_strengths.at[delimiter:(delimiter + size)].set(compute_audio_strengths_for_a_chunk(camera_pixel_microphone_time_offsets[delimiter:(delimiter + size), :], data_fft, data_frequencies))
